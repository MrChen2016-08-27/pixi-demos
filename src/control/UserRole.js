class UserRole{
   ...
   * @param  {Object} role            [获取角色]
   * @param  {Consumables Object} consumables     [当前消耗品模型]
   */
  getConsumables(role, consumables) {
    // 改变角色当前属性
    consumables.attrs.forEach((attr, index) => {
      // 消耗品改变后的数值
      const value = role[attr] + consumables.value[index];
      // 最大值max后首字母一般转为大写，如hp， 如果存在最大hp 则为 maxHp
      const maxValue = role[`max${attr.substring(0, 1).toUpperCase()}${attr.substring(1)}`];
